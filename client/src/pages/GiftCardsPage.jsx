...
import { featuresApi, fmtDate, money } from '../api/features.js';
        ...
        <div className="rounded-xl border bg-white p-4 space-y-3">
          ...
          {found && (
            <div className="text-sm bg-gray-50 rounded-lg p-3">
              Balance <b>{money(found.balance)}</b> of {money(found.initialBalance)} · {found.status}
              {found.expiresAt && <> · expires {fmtDate(found.expiresAt)}</>}
            </div>
          )}
          ...
          <tbody>
            {cards.map((c) => (
              <tr key={c.id} className="border-t">
                ...
                <td className="p-3">{money(c.initialBalance)}</td>
                <td className="p-3">{c.recipientEmail || '—'}</td>
                <td className="p-3 capitalize">{c.status}</td>
                <td className="p-3">{fmtDate(c.createdAt)}</td>
              </tr>
            ))}
