// server/db/schema.js (Drizzle ORM schema for reference)
import { pgTable, serial, text, integer, numeric, boolean, timestamp, jsonb, uuid, time, date, uniqueIndex } from 'drizzle-orm/pg-core';
...
export const businessUsers = pgTable('business_users', {
  ...
  createdAt: timestamp('created_at').defaultNow(),
}, (table) => ({ uniqueBusinessUser: uniqueIndex('unique_business_user').on(table.businessId, table.userId) }));
...
export const staff = pgTable('staff', {
  ...
  name: text('name').notNull(),
  title: text('title'),
  color: text('color').default('#6D28D9'),
...
}, (table) => ({ pk: uniqueIndex('service_add_ons_pk').on(table.serviceId, table.addOnId) }));
...
}, (table) => ({ pk: uniqueIndex('staff_services_pk').on(table.staffId, table.serviceId) }));
