// server/services/availabilityEngine.js (modified – added chain booking support)
...
/**
 * Get available slots for a single service.
 * (unchanged except internal helper extraction)
 */
...
/**
 * Chain booking: find compatible slots for a list of services (each can have different staff preferences).
 * @param {Object} options { businessId, services: [{serviceId, staffId?}], date, timezone, stepMinutes }
 */
export async function getChainSlots({ businessId, services, date, timezone = 'UTC', stepMinutes = 15 }) {
  if (!services || services.length === 0) return [];
  const dateTime = DateTime.fromISO(date, { zone: timezone }).startOf('day');

  // Get slots for each service individually (with staff preference)
  const slotArrays = await Promise.all(services.map(async ({ serviceId, staffId }) => {
  ...
  const firstServiceSlots = slotArrays[0].filter(slot => slot.isRecommended || slot.discountPercent > 0 || true); // keep all
  const chainCandidates = [];

  // Greedy approach: for each first slot, try to place subsequent services back-to-back with minimal gaps.
  for (const firstSlot of firstServiceSlots) {
    const chain = [firstSlot];
    let cursor = DateTime.fromISO(firstSlot.endAt).setZone(timezone);
    let feasible = true;

    for (let i = 1; i < services.length; i++) {
      const nextServiceSlots = slotArrays[i].filter(slot => {
        const slotStart = DateTime.fromISO(slot.startAt).setZone(timezone);
        // Allow up to 15 minutes gap between end of previous and start of next
        return slotStart >= cursor.minus({ minutes: 15 });
      });
      if (nextServiceSlots.length === 0) {
        feasible = false;
        break;
      }
      // Pick the slot with minimal gap to previous end
      nextServiceSlots.sort((a,b) => {
        const gapA = DateTime.fromISO(a.startAt).diff(cursor, 'minutes').minutes;
        const gapB = DateTime.fromISO(b.startAt).diff(cursor, 'minutes').minutes;
        return Math.abs(gapA) - Math.abs(gapB);
      });
      const chosen = nextServiceSlots[0];
      // Must start after previous slot's end with at most 15 min gap
      const chosenStart = DateTime.fromISO(chosen.startAt).setZone(timezone);
      if (chosenStart < cursor.minus({ minutes: 15 })) {
        feasible = false;
        break;
      }
      chain.push(chosen);
      cursor = DateTime.fromISO(chosen.endAt).setZone(timezone);
    }

    if (feasible && chain.length === services.length) {
      chainCandidates.push({
        totalPrice: chain.reduce((sum, s) => sum + s.price, 0),
        totalDurationMinutes: chain.reduce((sum, s) => sum + DateTime.fromISO(s.endAt).diff(DateTime.fromISO(s.startAt), 'minutes').minutes, 0),
        items: chain,
      });
    }
  }
  ...
  chainCandidates.sort((a,b) => a.totalPrice - b.totalPrice);
