...
router.post('/register', async (req, res, next) => {
  try {
    ...
    res.cookie('token', token, { httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production' })
      .status(201).json({ user: { id: user.id, email: user.email, name: user.name, roleGlobal: user.roleGlobal } });
  } catch (err) { next(err); }
});
...
router.post('/login', async (req, res, next) => {
  try {
    ...
    const token = generateAccessToken(user);
    res.cookie('token', token, { httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production' })
      .json({ user: { id: user.id, email: user.email, name: user.name, roleGlobal: user.roleGlobal } });
  } catch (err) { next(err); }
});
...
router.get('/session', optionalAuth, (req, res) => res.json({ user: req.user ?? null }));
