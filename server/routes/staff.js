...
router.post('/:staffId/services', authRequired, resolveBusinessFromUser, requireRole('owner','manager'), async (req, res, next) => {
  try {
    const { service_id, price_override, duration_override } = req.body;
    const result = await db.insert(staffServices).values({
      staffId: req.params.staffId, serviceId: service_id, priceOverride: price_override, durationOverride: duration_override,
    }).returning();
    res.status(201).json({ mapping: result[0] });
  } catch (err) { next(err); }
});
