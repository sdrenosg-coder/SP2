const DEFAULTS = {
  depositPercent: 20,
  highRiskDepositPercent: 50,
  noShowFeePercent: 50,
  minCancelHours: 24,
  depositForHighRisk: true,
};
...
/** riskScore: 0-100. Returns the amount to collect up front. */
export function computeDeposit({ price, depositRequired, riskScore = 0, business }) {
  ...
  if (p.depositForHighRisk && riskScore >= 70 && p.highRiskDepositPercent > pct) {
    pct = p.highRiskDepositPercent;
    reason = 'high_risk';
  }
